...
<div class="container" style="margin-top: 50px;">
    ...
    @endif
    <hr>
        <div class="card-header text-center" style="font-weight: 600;font-style: italic;font-size: xx-large;">{{ __('Les reservations de voyages') }}</div>
    <hr>
                ...
                @if(count($reservations) > 0)
                    @foreach($reservations as $reservation)
                        <tr>
                            ...
                            <td>{{ $reservation->ville_voyage }}</td>
                            <td>{{ $reservation->date_debut_voyage }}</td>
                            <td>{{ $reservation->date_fin_voyage }}</td>
                            ...
                            <td>
                                <a href="" data-bs-toggle="modal" data-bs-target="#confirm-delete-modal-{{ $reservation->id }}" style="color: #f90606;font-family: monospace;font-weight: bold;font-size: large;text-decoration: none">Annuler</a>
                                {{-- <i data-bs-toggle="modal" data-bs-target="#confirm-delete-modal-{{ $reservation->id }}" class="fa-solid fa-trash" style="color: #f90606;">Annuler</i> --}}
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <form style="box-shadow: none" method="POST" action="{{ route('home.voyage.reservation.destroy',$reservation->id) }}">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            <td>
                                @if($reservation->status == 'acceptée')
                                    <form action="{{ route('imprimer.reservation.voyage') }}" method="POST" target="_blank">
                                        @csrf
